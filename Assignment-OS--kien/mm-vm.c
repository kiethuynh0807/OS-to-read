...
struct vm_area_struct *cur_vma = get_vma_by_num(caller->krnl->mm, vmaid);
    ...
    /* 1. Align the increment size to page size */
    // Ensure we allocate full pages even if inc_sz is small
    addr_t inc_amt = PAGING_PAGE_ALIGNSZ(inc_sz);
    addr_t new_end = old_end + inc_amt;
    int incnumpage = inc_amt / PAGING_PAGESZ;

    /* 2. Validate overlap */
    /* Check if the new extended range hits another VMA */
    if (validate_overlap_vm_area(caller, vmaid, old_end, new_end) < 0) {
        return -1; 
    }

    /* 3. Update VMA boundary */
    cur_vma->vm_end = new_end;

    /* 4. Map to Physical RAM */
    /* We map the newly created range [old_end, new_end] to RAM */
    if (vm_map_ram(caller, old_end, new_end, old_end, incnumpage, NULL) < 0) {
